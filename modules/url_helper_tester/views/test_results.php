...
<body>
  <div class="container">
    <div class="card">
      <div class="card-heading">URL Helper Unit Tests</div>
      <div class="card-body">
        <p>Comprehensive tests for all <code>url_helper</code> functions.</p>
        ...
        // Official docs order
        $docs_order = [
          'anchor', 'current_url', 'get_last_segment', 'get_num_segments',
          'previous_url', 'redirect', 'remove_query_string', 'segment',
        ];
          ...
          <div class="test-group">
            ...
            <?php foreach ($rows as $row):
              ...
              $exp = is_bool($row['expected']) ? ($row['expected'] ? 'true' : 'false')
                   : (is_array($row['expected']) ? '[]' : (string)$row['expected']);
              $act = is_bool($row['actual']) ? ($row['actual'] ? 'true' : 'false')
                   : (is_array($row['actual']) ? '[]' : (string)$row['actual']);
